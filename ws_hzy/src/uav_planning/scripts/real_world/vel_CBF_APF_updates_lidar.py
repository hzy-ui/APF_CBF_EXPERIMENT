...
if __name__ == "__main__":
	...
	#############################
        if rospy.get_time() - time_startup <= 2000:
                vel.twist.linear.x = 0
                vel.twist.linear.y = 0
                vel.twist.linear.z = 0 #K1 *  (init_z - flight_pose.pose.position.z)
                rospy.loginfo("ACCENDING... " + str(rospy.get_time() - time_startup))
        
        elif rospy.get_time() - time_startup > 2000 and rospy.get_time() - time_startup < 10000:
                x = (flight_pose.pose.position.x, flight_pose.pose.position.y)
                t = rospy.get_time() - (20 + time_startup) 
                # print(t)
                v1, v2, h, gamma, b= CBF_controller_updates(x, x_initial, x_goal, R, rho, k, t_star, t)
                ...
                if math.sqrt(u1**2 + u2**2) >= 0.1:
                    u1 = math.sqrt(0.1) * ( u1/math.sqrt(u1**2 + u2**2) )
                    u2 = math.sqrt(0.1) * ( u2/math.sqrt(u1**2 + u2**2) )
